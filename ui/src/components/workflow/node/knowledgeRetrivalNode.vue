...
function onNodeDelete(ev) {
  const crawlerNodeData = props.data["retrieveKnowledgeBaseNodeData"];
...
<template>
  <Card :title="node.data['name']" :hoverable="true" :body-style="{padding:'10px'}" :head-style="{padding:'10px'}">
    ...
    <node-variable-display :input-variables="data['retrieveKnowledgeBaseNodeData'].inputVariables"
                           :output-variables="data['retrieveKnowledgeBaseNodeData'].outputVariables"
                           :has-output="true"
                           :has-input="true"/>
  </Card>
</template>
