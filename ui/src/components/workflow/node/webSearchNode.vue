...
function onNodeDelete(ev) {
  const webSearchNodeData = props.data["webSearchNodeData"];
  NodeUtil.resetInputVariableRef(webSearchNodeData, id=>ev.detail.id === id);
}

function onEdgeDelete(ev) {
  const prevNodes = NodeUtil.getPrevNodes(props.id, getNodes.value, getEdges.value).map(node=>node.id);
  const webSearchNodeData = props.data["webSearchNodeData"];
  NodeUtil.resetInputVariableRef(webSearchNodeData, id=>!prevNodes.includes(id));
}
...
<template>
  <Card :title="node.data['name']" :hoverable="true" :body-style="{padding:'10px'}" :head-style="{padding:'10px'}">
    ...
    <node-variable-display :input-variables="data['webSearchNodeData'].inputVariables"
                           :output-variables="data['webSearchNodeData'].outputVariables"
                           :has-output="true"
                           :has-input="true"/>
  </Card>
</template>
